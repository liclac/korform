...
class SheetColumn(models.Model):
    ...
    def render(self, member):
        keys = self.key.split(',')
        format_string = self.format_string or u" ".join([u"{%i}" % i for i in range(len(keys))])
        ...
        args = [ kwargs.get(key.strip(), '') for key in keys ]
        
        formatter = FieldFormatter()
        value = formatter.vformat(format_string, args, kwargs)
